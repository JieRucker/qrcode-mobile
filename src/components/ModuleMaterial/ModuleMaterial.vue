...
<template>
  <!-- 素材 -->
  <div class="moduleMaterial">
    <div v-if="show0" class="show0">
      <component :is="item.component" :params="item.params" v-for="(item,index) in componentList"
                 :key="index"></component>

    </div>
    <div v-if="show1" class="show1">
      <cell class="cell" :title="linkName" :link="{path:'./MaterialPage',query:{templateId:templateId}}"></cell>
    </div>
  </div>
</template>
